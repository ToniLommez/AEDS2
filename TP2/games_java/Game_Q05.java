...
class ListaGames {
    ...
    public void insertStart(Games game) {
    ...
    public void insertMiddle(Games game, int pos) {
    ...
    public Games removeEnd() {
        Games game = this.games[this.n_games-1];
        this.n_games--;
        return game;
    }

    public Games removeMiddle(int pos) {
        Games game = this.games[pos];
        for (int i = pos; i < this.n_games-1; i++) {
            this.games[i] = this.games[i+1];
        }
        this.n_games--;
        return game;
...
public class Game {
    ...
    public static Games procurarGame(Games game[], int id){
        Games temp = game[0];
        for (int i = 0; i < game.length; i++) {
            if (game[i].get_app_id() == id) {
                temp = game[i];
                i = game.length;
            }
        }
    ...
    public static void main(String[] args) throws IOException {
        ...
        try {
            ...
            for (int i = 0; i < numEntrada; i++) {
                ...
                temp += entrada[i].charAt(1);

                if (temp.equals("II")) {
                    temp = new String();
                    for (int j = 3; j < entrada[i].length(); j++) {
                        temp += entrada[i].charAt(j);
                    }
                    pos = Integer.parseInt(temp);
                    lista.insertStart(procurarGame(games, pos));
                } else if (temp.equals("I*")) {
                    temp = new String();
                    temp += entrada[i].charAt(3);
                    temp += entrada[i].charAt(4);
                    pos = Integer.parseInt(temp);
                    temp = new String();
                    for (int j = 6; j < entrada[i].length(); j++) {
                        temp += entrada[i].charAt(j);
                    }
                    lista.insertMiddle(procurarGame(games, Integer.parseInt(temp)), pos);
                    ...
                    pos = Integer.parseInt(temp);
                    lista.insertEnd(procurarGame(games, pos));
                } else if (temp.equals("RI")) {
                    MyIO.print("(R) ");
                    MyIO.print(lista.removeStart().get_name());
                    MyIO.print("\n");
                    ...
                    pos = Integer.parseInt(temp);
                    MyIO.print("(R) ");
                    MyIO.print(lista.removeMiddle(pos).get_name());
                    MyIO.print("\n");
                } else if (temp.equals("RF")) {
                    MyIO.print("(R) ");
                    MyIO.print(lista.removeEnd().get_name());
                    MyIO.print("\n");
                }
            }
            lista.printall();
