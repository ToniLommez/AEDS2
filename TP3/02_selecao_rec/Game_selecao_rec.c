...
// ordenando por nome
void selection_sort_rec(Game game[], int n_games, int *comparacoes, int *movimentacoes, int i)
{
    Game temp;
    if (i < n_games - 1)
    {
        ...
        *movimentacoes += 3;
        ...
        selection_sort_rec(game, n_games, comparacoes, movimentacoes, i + 1);
    }
}
...
int main(void)
{
    ...
    clock_t t1 = clock();
    // ordenando
    selection_sort_rec(lista, lista_size, &comparacoes, &movimentacoes, 0);
    // calculando tempo
    clock_t t2 = clock();
