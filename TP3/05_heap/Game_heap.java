...
class ListaGames {
    ...
    boolean eMaiorHeap(int a, int b) {
        ...
        if (ano_a > ano_b) {
            ...
            this.comparacoes += 1;
            if (ano_a == ano_b && mes_a == mes_b && this.games[a].get_dia() > this.games[b].get_dia()) {
                eMaior = true;
            } else {
                
                if (ano_a == ano_b && mes_a == mes_b && this.games[a].get_dia() == this.games[b].get_dia()) {
                    this.comparacoes += 1;
                    if (this.games[a].get_name().compareTo(this.games[b].get_name()) > 0) {
                        eMaior = true;
                    }
                }
            }
...
public class Game_heap {
    ...
    public static void matriculaHeapsort(int comparacoes, int movimentacoes, long tempo) {
        try {
            Formatter file = new Formatter("matricula_heapsort.txt");
    ...
    public static void main(String[] args) throws IOException {
        ...
        try {
            ...
            // criando log
            matriculaHeapsort(lista.comparacoes, lista.movimentacoes, total_time);
            // printando
            for (int i = 0; i < numEntrada; i++) {
