...
class Games {
     ...
     * Metodo de clonagem
     */
    public void clone(Games toClone) {
        toClone.set_app_id(this.app_id);
        toClone.set_name(this.name);
        toClone.set_release_date(this.release_date);
        toClone.set_owner(this.owner);
        toClone.set_age(this.age);
        toClone.set_price(this.price);
        toClone.set_dlcs(this.dlcs);
        toClone.set_website(this.website);
        toClone.set_windows(this.windows);
        toClone.set_mac(this.mac);
        toClone.set_linux(this.linux);
        toClone.set_upvotes(this.upvotes);
        toClone.set_avg_pt(this.avg_pt);
        toClone.set_developers(this.developers);

        if (this.languages != null) {
            toClone.set_new_languages(this.languages.length);
            for (int i = 0; i < this.languages.length; i++) {
                toClone.set_languages(this.languages[i], i);
            }
        }

        if (this.genres != null) {
            toClone.set_new_genres(this.genres.length);
            for (int i = 0; i < this.genres.length; i++) {
                toClone.set_genres(this.genres[i], i);
            }
            toClone.set_genres(this.genres);
        }
    }
}
...
class ListaGames {
    ...
    ListaGames(int tamanho) {
        this.primeiro = this.ultimo = new Games();
        Games last = this.primeiro;
        int i = 0;
        while (i < tamanho) {
        ...
        last.set_next(this.primeiro);
    }

    /* METODOS PARA LISTA ENCADENADA SIMPLES */
    ...
    public void insertCircular(Games novo) {
        novo.clone(this.ultimo);
        if (this.ultimo.get_next() == this.primeiro) {
            this.primeiro = this.primeiro.get_next();
        }
        this.ultimo = this.ultimo.get_next();
        printAllAvgPt();
    }

    public Games removeCircular() {
        Games removido = this.primeiro;
        this.primeiro = this.primeiro.get_next();
        return removido;
    }

    public void printAllAvgPt() {
        Games tmp = this.primeiro;
        double total = 0;
        int numValores = 0;
        while (tmp != this.ultimo) {
            total += tmp.get_avg_pt();
            numValores++;
            tmp = tmp.get_next();
        }
        System.out.printf("%d\n", (int)Math.round(total/numValores));
    }
    ...
    public void printAll() {
        ...
        int i = 0;
        while (tmp != this.ultimo) {
            System.out.printf("[%d] %s", i, tmp.toString());
            tmp = tmp.get_next();
        }
    }
...
public class Game {
    ...
    public static void main(String[] args) throws IOException {
        ...
        ListaGames lista = new ListaGames(5);
        ...
        try {
            ...
            for (int i = 0; i < numEntrada; i++) {
                lista.insertCircular(listaInicial.cloneById(Integer.parseInt(entrada[i])));
            }
            ...
            for (int i = 0; i < numEntrada; i++) {
                ...
                if (temp.equals("I")) {
                    ...
                    lista.insertCircular(listaInicial.cloneById(id));
                } else if (temp.equals("R")) {
                    MyIO.print("(R) ");
                    MyIO.print(lista.removeCircular().get_name());
                    MyIO.print("\n");
                }
            }
            lista.printAll();
