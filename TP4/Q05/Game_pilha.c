...
Pilha* novaPilha(struct games game) {
...
void insertStart(Pilha* topo, struct games game)
{
    Pilha* novo = novaPilha(game);
    novo->next = topo->next;
    novo->game = game;
    topo->next = novo;
}

struct games removeStart(Pilha* topo)
{
    Pilha* trash = topo->next;
    struct games game = trash->game;
...
int printAllRec(Pilha* atual, int i)
{
    if (atual != NULL)
    {
        i = printAllRec(atual->next, i);
        printf("[%d] ", i);
        toString(atual->game);
        printf("\n");
    }
...
void printAll(Pilha* topo)
{
    printAllRec(topo->next, -1);
}
...
int main()
{
    ...
    for (int i = 0; i < numEntrada; i++)
    {
        ...
        {
            temp[0] = '\0';
            for (int j = 2; j < strlen(entrada); j++)
            {
                temp[j - 2] = entrada[j];
                temp[j - 1] = '\0';
            }
            pos = atoi(temp);
            insertStart(topo, procurarGame(game, pos));
        }
        else if (temp[0] == 'R')
        {
            printf("(R) %s\n", removeStart(topo).name);
        }
    }
