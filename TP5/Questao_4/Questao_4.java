...
class No {
    ...
    public No(Games valor, boolean cor) {
        this.valor = valor;
        this.cor = cor;
        this.esq = null;
        this.dir = null;
    ...
    public No(Games valor, No esq, No dir, boolean cor) {
        this.valor = valor;
        this.cor = cor;
        this.esq = esq;
        this.dir = dir;
...
class Alvinegra {
    ...
    public void inserir(Games novo) {
        if (raiz == null) {
            raiz = new No(novo);
        } else if (raiz.getEsq() == null && raiz.getDir() == null) {
            if (raiz.eMenor(novo)) {
                raiz.setEsq(new No(novo));
            } else {
                raiz.setDir(new No(novo));
            }
        } else if (raiz.getEsq() == null) {
            if (raiz.eMenor(novo)) {
                raiz.setEsq(new No(novo));
            } else if (raiz.getDir().eMenor(novo)) {
                raiz.setEsq(new No(raiz.getValor()));
                raiz.setValor(novo);
            } else {
                raiz.setEsq(new No(raiz.getValor()));
                raiz.setValor(raiz.getDir().getValor());
                raiz.getDir().setValor(novo);
            }
            ...
            if (raiz.eMaior(novo)) {
                raiz.setDir(new No(novo));
            } else if (raiz.getEsq().eMaior(novo)) {
                raiz.setDir(new No(raiz.getValor()));
                raiz.setValor(novo);
            } else {
                raiz.setDir(new No(raiz.getValor()));
                raiz.setValor(raiz.getEsq().getValor());
                raiz.getEsq().setValor(novo);
            }
            raiz.getEsq().setCor(false);
            raiz.getDir().setCor(false);
        } else {
            inserir(novo, null, null, null, raiz);
        }
        raiz.setCor(false);
    }

    private void inserir(Games novo, No bisavo, No avo, No pai, No i) {
        if (i == null) {
            if (pai.eMenor(novo)) {
                pai.setEsq(new No(novo, true));
                i = pai.getEsq();
            } else {
                pai.setDir(new No(novo, true));
                i = pai.getDir();
            }
            if (pai.isCor()) {
            ...
            if (i.eMenor(novo)) {
                inserir(novo, avo, pai, i, i.getEsq());
            } else if (i.eMaior(novo)) {
                inserir(novo, avo, pai, i, i.getDir());
            } else {
                System.out.printf("Erro inserir (elemento repetido)!");
            }
        }
    }
    ...
    private void balancear(No bisavo, No avo, No pai, No i) {
        if (pai.isCor()) {
            if (avo.eMaior(pai.getValor())) {
                if (pai.eMaior(i.getValor())) {
                    avo = rotacaoEsq(avo);
                } else {
                    avo = rotacaoDirEsq(avo);
                }
            } else {
                if (pai.eMenor(i.getValor())) {
                    avo = rotacaoDir(avo);
                } else {
            ...
            if (bisavo == null) {
                raiz = avo;
            } else if (bisavo.eMenor(avo.getValor())) {
                bisavo.setEsq(avo);
            } else {
                bisavo.setDir(avo);
            }
...
public class Questao_4 {
    ...
    public static void matriculaAlvinegra(long tempo, int comparacoes) {
        try {
            Formatter file = new Formatter("matricula_avinegra.txt");
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; populacao[i] != null; i++) {
            alvinegra.inserir(encontrarGame(Integer.parseInt(populacao[i]), arrayGames));
        }
        ...
        for (int i = 0; comandos[i] != null; i++) {
            if (comandos[i].charAt(0) == 'I') {
                alvinegra.inserir(encontrarGame(Integer.parseInt(comandos[i].substring(2)), arrayGames));
            } else if (comandos[i].charAt(0) == 'R') {
                // alvinegra.remover(comandos[i].substring(2));
            }
        }
        
        for (int i = 0; pesquisas[i] != null; i++) {
            System.out.printf("%s\n", pesquisas[i]);
            alvinegra.pesquisar(pesquisas[i]);
        }

        long end = new Date().getTime();
        long tempo = end - start;
        matriculaAlvinegra(tempo, alvinegra.getComparacoes());
    }
}
