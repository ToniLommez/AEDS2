...
class Hash {
    ...
    public int h(Games elemento) {
        return Math.abs(hashCode(elemento.get_name()) % this.m1);
    }

    public int h(String nome) {
        return Math.abs(hashCode(nome) % this.m1);
    }
    ...
    public int pesquisar(String nome) {
        int pos = h(nome);
        int resp = -1;
        if (tabela[pos] != null && tabela[pos].get_name().compareTo(nome) == 0) {
            comparacoes += 1;
            resp = pos;
        } else if (tabela[pos] != null) {
            comparacoes += 2;
            for (int i = 0; i < reserva; i++) {
                if (tabela[m1 + i].get_name().compareTo(nome) == 0) {
                    resp = m1 + i;
                    i = reserva;
                }
            }
    ...
    public void showAll() {
        for (int i = 0; i < tabela.length; i++) {
            System.out.printf("%d \t%s\n", i, (tabela[i] != null) ? tabela[i].get_name() : "");
        }
    }
...
public class Questao_5 {
    ...
    public static boolean isFim(String s) {
        return (s != null && s.length() == 3 && s.charAt(0) == 'F' && s.charAt(1) == 'I' && s.charAt(2) == 'M');
    }
    ...
    public static void matriculaHashReserva(long tempo, int comparacoes) {
        try {
            Formatter file = new Formatter("matricula_hashReserva.txt");
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; comandos[i] != null; i++) {
            System.out.printf("=> %s\n%s\n", comandos[i], (hash.pesquisar(comandos[i]) != -1) ? "Posicao: " + Integer.toString(hash.pesquisar(comandos[i])) : "NAO");
        }

        long end = new Date().getTime();
        long tempo = end - start;
        matriculaHashReserva(tempo, hash.getComparacoes());
    }
}
