...
class Hash {
    ...
    public int pesquisar(String nome) {
        ...
        if (tabela[pos] != null && tabela[pos].get_name().compareTo(nome) == 0) {
            ...
            int newPos = reh(pos);
            comparacoes += 2;
            if (tabela[newPos].get_name().compareTo(nome) == 0) {
                pos = newPos;
                resp = pos;
            }
        }
...
public class Questao_6 {
    ...
    public static void matriculaHashRehash(long tempo, int comparacoes) {
        try {
            Formatter file = new Formatter("matricula_hashRehash.txt");
    ...
    public static void main(String[] args) throws IOException {
        ...
        matriculaHashRehash(tempo, hash.getComparacoes());
    }
}
